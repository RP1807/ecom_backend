...
def signin(request):
    if not request.method == 'POST':
        return JsonResponse({"error": "Send a POST method with valid params only"})
    ...
    try:
        ...
        if user.check_password(password):
            user_dict = user_model.objects.filter(email=username).values().first()
